...
public class ContribuinteTarefaDAO {

	public int inserirContribuinteTarefa(ContribuinteTarefa contribuinteTarefa) throws IOException {
		int id = -1;
		String sql = "INSERT INTO contribuintes_tarefas (contribuinte_id, tarefa_id)"
					+ "values (?,?);";

		try (Connection conn = ConnectionFactory.getConnection(); PreparedStatement pst = conn.prepareStatement(sql);) {

			pst.setInt(1, contribuinteTarefa.getContribuinte().getId());
			pst.setInt(2, contribuinteTarefa.getTarefa().getId());
			pst.execute();
